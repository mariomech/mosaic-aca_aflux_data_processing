...
;====================================================================================================================================================
@/projekt_agmwend/data/MOSAiC_ACA_S/Pangaea_Files_MOSAiC_ACA/ncdf_rdwr.pro
@/projekt_agmwend/data/MOSAiC_ACA_S/Pangaea_Files_MOSAiC_ACA/get_eagle_times.pro
@/projekt_agmwend/data/MOSAiC_ACA_S/Pangaea_Files_MOSAiC_ACA/read_eagle.pro

CLOSE,/all

l = path_sep()

ncdf_path = '/projekt_agmwend2/data_raw/MOSAiC_raw/Pangaea_Files/NetCDF_files/AisaHAWK/'
...
dates   = ['20200902a','20200904a','20200907a','20200908a','20200910a','20200911a','20200913a'];



MOSAiC_path = '/projekt_agmwend/data/MOSAiC_ACA_S/'



flight_dirs = MOSAiC_path + 'Flight_' + dates + '/AisaHAWK/'
...
;d=0
    ...
    ;Smart navigation,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,
    raw_path = prefix_raw + 'data_raw/MOSAiC_ACA_S_raw/'+ strjoin(parts(3)) + l
...
;       IF flights(d) EQ '04' THEN  hawk_time = hawk_time - 2.
       ...
       IF f EQ 0 THEN BEGIN
          ...
          ncdf_attput,id,/GLOBAL,'Comment1','Partly, radiance data for the central spatial pixels might be slightly affected by condensation on the lens. In case of condensation, it is identified by a stripe of slightly lower radiance values in the center of the image.'

           ncdf_attput,id,/GLOBAL,'Comment2','Michael Schaefer ' + systime()
;

              varID_wl = ncdf_vardef(id,'wavelength',[dim0], GZIP=9)
              ncdf_attput, id, VarId_wl, 'Long name', 'Wavelength'
              ncdf_attput, id, VarId_wl, 'Units','nm'
       ENDIF


       IF max(img_hawk) LT 32767 THEN varID_img = ncdf_vardef(id,'radiance_' + hour,[dim1,dim0,dim2],/short, GZIP=9) $
       ELSE varID_img = ncdf_vardef(id,'radiance_' + hour,[dim1,dim0,dim2],/long, GZIP=9)

       ncdf_attput, id, VarId_img, 'Long name', 'Upwards radiance'
       ncdf_attput, id, VarId_img, 'Units','1E-4 W m-2 sr-1 nm-1'


       varID_time = ncdf_vardef(id,'time_' + hour,[dim2], GZIP=9)
       ncdf_attput, id, VarId_time, 'Long name', 'Decimal time'
       ncdf_attput, id, VarId_time, 'Units','UTC hour'


       varID_lat = ncdf_vardef(id,'lat_' + hour,[dim2], GZIP=9)
       ncdf_attput, id, VarId_lat, 'Long name', 'Latitude'
       ncdf_attput, id, VarId_lat, 'Units','Degrees'

       varID_lon = ncdf_vardef(id,'lon_' + hour,[dim2], GZIP=9)
       ncdf_attput, id, VarId_lon, 'Long name', 'Longitude'
       ncdf_attput, id, VarId_lon, 'Units','Degrees'

       varID_roll = ncdf_vardef(id,'roll_' + hour,[dim2], GZIP=9)
       ncdf_attput, id, VarId_roll, 'Long name', 'Roll angle'
       ncdf_attput, id, VarId_roll, 'Units','Degrees'

       varID_pitch = ncdf_vardef(id,'pitch_' + hour,[dim2], GZIP=9)
       ncdf_attput, id, VarId_pitch, 'Long name', 'Roll angle'
       ncdf_attput, id, VarId_pitch, 'Units','Degrees'

       varID_yaw = ncdf_vardef(id,'yaw_' + hour,[dim2], GZIP=9)
