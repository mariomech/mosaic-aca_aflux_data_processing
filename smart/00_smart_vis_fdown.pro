...
openr,1,'F:\campaigns\mosaic-aca\data\smart\idl\config_quicklook.txt'
line=strarr(5)
readf,1,line
...
datum='200907'
sav_suffix='_fdw_210521_vis_diffus.sav'

restore, file='F:\campaigns\mosaic-aca\data\libradtran\'+datum+'\BBR_DirectFraction_Flight_20'+datum+'a_spectral_R0.sav'; dd, time_pos, lon_pos, lat_pos, fglo, wl
...
for i=0, n_elements(time_dg)-1 do begin
    ...
    dg_fit(i,*)=poly_fit(wl(finde),(1-dd(i,finde)),8)
...
gesamtlauf=0
time_all=fltarr(30000)
spec_all=fltarr(30000,pixel)
hh_all=fltarr(30000)
mm_all=fltarr(30000)
ss_all=fltarr(30000)
height_all=fltarr(30000)
sza_all=fltarr(30000)
saa_all=fltarr(30000)
...
;fdown
if id_spectroButton eq 0 and id_rangebutton eq 0 then calibfile=directory_calib+'calfac_fdn_VIS_1000ms_210428_nachkalib.dat';calfac_fdn_VIS_500ms_210503_nachkalib_transfer_from200901.dat';

if id_rangebutton eq 0 then begin ; für das VIS

    TIMECOUNTER+=SYSTIME(1)-T000
    absfac=read_ASCII(calibfile)
    T000=SYSTIME(1)
    absfac=absfac.(0)
...
if id_rangebutton eq 1 then begin ; für nir
endif
...
;dc2=[     0.996898,    0.00451946, -0.00141528,  6.45696e-005,-1.04718e-006,  5.76522e-009]
dc2=[1.00119, -0.00378, -4.11376E-4, 2.55452E-5, -4.47974E-7, 2.57825E-9]

;SZA cosine correction for VN7 - 2021
dc7=[1.00001,-0.00724535,0.000854550,-5.69375e-005,1.78234e-007,1.31599e-007,-5.66740e-009,1.02999e-010,-8.84844e-013, 2.95186e-015]
...
; Wavelength-polynome diffuse correction for VN2 - nicht mehr benutzt 28.8.17
...
diff_corr[*]=1.02 ; wellenlängenunabhängig gesetzt 30.04.20
...
if cloud eq 0 then begin
    ...
    d_g=fltarr(n_elements(time),1024)
    ...
    for ii=0, n_elements(time)-1 do begin
        ...
        finde=where(d_g lt 0.025)
        if finde(0) gt -1 then d_g(finde)=0.025
    endfor
...
saa_all=saa_all(0:gesamtlauf-1,*)
