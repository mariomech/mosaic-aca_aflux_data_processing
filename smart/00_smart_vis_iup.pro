...
@M:\campaigns\ac3\acloud\smart\idl\einlesen_vis_polar.pro
...
hh_files=['09','10','11','12','13','14']
filename=file_search('T:\data_raw\MOSAiC_ACA_S_raw\Flight_20200913a\horidata\*.nav')
...
data_dir=DIALOG_PICKFILE(path=data_path,/directory,title='Choose data directory')
close,/ALL
...
;I down (VIS B-Ch1, B1, ASP06C5B), I up (VIS B-Ch2, F2, 10b)
;F down (VIS A-Ch1, NIR B, VN2, 2b), F up (VIS A-Ch2, NIR A, VN4, 9b)

if id_spectroButton eq 0 and id_rangebutton eq 0 then filters='*VIS-A_ch2*'; irradiance VIS

if id_spectroButton eq 1 and id_rangebutton eq 0 then filters='*VIS-B_ch2*.dat'; radiance VIS

if id_spectroButton eq 0 and id_rangebutton eq 1 then filters='*nir-Ch1.dat'; irradiance NIR

if id_spectroButton eq 1 and id_rangebutton eq 1 then filters='*NIR.dat'; radiance NIR

if id_spectroButton eq 2 then filters='*pyrgeometer.dat'

for hhfiles=0, n_elements(hh_files)-1 do begin ; loop über einzelne stunden

hhsuch=hh_files(hhfiles)
daystring1=strmid(ymd,4,2)+strmid(ymd,2,2)+strmid(ymd,0,2)
files=file_search(data_dir+'ALBEDO_'+daystring1+'_'+hhsuch+'*.VIS-B_ch2.dat') ; neu
...
if id_rangebutton eq 1 then einlesen_nir_polar,filename1,hh,mm,ss,spec,tint


finde=strpos(filename1,'ALBEDO_')
yy=fix(strmid(filename1,finde+7,2))
month=fix(strmid(filename1,finde+9,2))
dd=fix(strmid(filename1,finde+11,2))
...
; fup
if id_spectroButton eq 1 and id_rangebutton eq 0 then calibfile=directory_calib+'500ms_Iup_vis_210409.dat'
...
if id_rangebutton eq 0 then begin ; für VIS Spektro radiance or irradiance
    ...
    for i=0,ntotal-1 do begin
        darkcurrent=mean(spec[i,35:darkpixel]) ;35 test
        spec[i,*]=(spec[i,*]-darkcurrent)/(tint[i]/500.)*absfac ; kalfak war für 500 ms bestimmt
    endfor
...
if id_rangebutton eq 1 then begin ; für NIR Spektro radiance or irradiance
    ...
    ntotal=n_elements(tint)
    for i=0,ntotal-1 do begin
        calibfile=directory_calib+'kalfak_nir_'+strtrim(string(fix(tint[i])),2)+'ms_'+strmid(data_path,6,6,/reverse_offset)+'_ohne_tc.dat'
        absfac=read_ascii(calibfile, comment_symbol='#')
        wl=absfac.(0)[0,*]
        absfac=absfac.(0)[1,*]
...
if id_spectrobutton eq 0 then begin
    ...
    ;SZA cosine correction for VN2
    dc2=[1.00069,-9.69408E-3,1.74351E-3,-2.68498E-4,1.79312E-5,-6.17242E-7,1.21065E-8,-1.37711E-10,8.4968E-13,-2.2039E-15]
    ...
    ;fc4=[-41.22209,0.59364,-3.70303E-3,1.33568E-5,-3.06075E-8,4.61306E-11,-4.56888E-14,2.86646E-17,-1.03369E-20,1.63275E-24] ; alt
    fc4=[5.5982182175E-001,     1.4599878923E-003,    -1.4647935132E-006,    -3.5503111562E-010,     7.3234083728E-013,     4.5642476242E-016,    -4.4342128260E-019,0,0,0]; neu vorkalib acloud
    ...
    if sensor eq 'VN2' or sensor eq 'irr_A' then begin
    ;    for i=0,ntotal-1 do begin
        for i=0,ntotal-1 do begin
         ...
         if sensor eq 'VN2' then begin
            for j=0,9 do begin
              dir_corr[i,*]+=dc2[j]*sza[i]^j
            endfor
         endif
    ...
    diff_corr[*]=0.93 ; VN4 ACLOUD VIS


    ; ----------------------------------
    d_g=findgen(pixel)

    ; verdi 23.4.12 2045-2100, clear sky
           a1=8.2388
           b1=-0.00783
           c1=0.00365
    ...
    print,'i'
    d_g[i]=dif_glo(wl[i],a1,b1,c1)
    endfor
    ; -----------------------------------


    ; Diffuskorrektur abwärtsschauender Einlass

    if sensor eq 'VN4' then begin
    for i=0,pixel-1 do begin
      spec[*,i]=spec[*,i]*diff_corr[i] ; constant unterhalb von 400 nm
    endfor
...
outputfile=output_path+strtrim(string(min(time_all)/3600.),2)+'_'+strtrim(string(max(time_all)/3600.),2)+'_iup_210409_vis.sav'
save, file=outputfile, wl, time_all, spec_all
...
if id_spectrobutton eq 0 then begin
  ...
  if id_spectrobutton eq 1 then begin
   ...
   heading4=widget_text(plotting_base, $
     XSIZE=10, YSIZE=1,xoffset=630,yoffset=17,value='Time Series')
   textcomment=widget_text(plotting_base, $
     XSIZE=50, YSIZE=7,xoffset=880,yoffset=25,value=['Quicklook Window for Sensor =   '+ spectro ,'date = '+strtrim(string(dd),2)+'.'+strtrim(string(month),2)+'.'+strtrim(string(yy),2),'','','program version 09.04.2020'])
endif
