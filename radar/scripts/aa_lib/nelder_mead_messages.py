...
def get_exit_limit_line(record, options, parameter):
    ...
    name = name[:cws[0] - 1]
    line = su.color('bold') + color + ('%s:' % name).ljust(cws[0]) + su.color(None) + color + ('{:>{cw}s}').format('%s / %s ' % (val_str, val_max_str), cw=cws[1] - 2) + ' %s' % progress_bar + ' ( %s )' % annotation + '\n'
    return line
...
def get_parameter_line(record, options, n=-1):
    ...
    line = su.color('bold') + color + (name + ':').ljust(cws[0]) + su.color(None) + color + (u'{:>{cw}s} ').format(val_unc_str(val0, unc0) + ' |', cw=cws[1]) + (u'{:>{cw}s} ').format(val_unc_str(val, unc) + ' |', cw=cws[2] - 1) + (u'{:>{w}s} ').format(hum_fmt(tol), w=6) + ' %s' % progress_bar + '\n'
    return line
...
def display_termination_message(record, options=None):
    message = 'Succes                  : %s\n' % record['success'] + 'Terminatation reason    : %s\n' % record['message'] + 'Result                  : %s\n' % record['x'] + '- uncertainty           : %s\n' % record['x_unc'] + 'Function value (result) : %f\n' % record['f'] + '- uncertainty           : %f\n' % record['f_unc'] + 'Ititial guess           : %s\n' % record['x0'] + '- uncertainty           : %s\n' % record['x0_unc'] + 'Ititial function value  : %f\n' % record['f0'] + 'Iterations              : %i\n' % record['Niter'] + 'Function evaluations    : %i\n' % record['Nfev'] + ''
    print(message)
...
def get_action_history_section(record, options):
    ...
    line_footer = '(%sI%s)nitialize' % (colors[0], endc) + ', (%sS%s)hrink' % (colors[1], endc) + ', (%sC%s)ontract' % (colors[2], endc) + ', (%sR%s)eflect' % (colors[3], endc) + ', (%sE%s)xpand' % (colors[4], endc) + su.color(None) + '\n'
    line_char = ''
    line_sym = ''
