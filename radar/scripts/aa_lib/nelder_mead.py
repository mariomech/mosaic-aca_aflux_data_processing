...
# Embedded file name: /home/anhaeus/acloud/aa_utils/processing/aa_lib/nelder_mead.py
...
import aa_lib.nelder_mead_messages as messages
_alpha = 1.0
_gamma = 2.0
_rho = 0.5
_sigma = 0.5
...
def minimize(fun, x0, args=(), method='nelder-mead', jac=None, hess=None, hessp=None, bounds=None, constraints=(), tol=None, callback=None, options=None):
    if method.lower() != 'nelder-mead':
        raise NotImplementedError()
    return minimize_nelder_mead(fun, x0, args=args, callback=callback, **options)


def minimize_nelder_mead(fun, x0, args=(), callback=None, maxiter=None, maxfev=None, maxtime_sec=None, disp=False, return_all=False, initial_simplex=None, xatol=None, fatol=None, adaptive=False, x0_uncert=None, alpha=_alpha, gamma=_gamma, rho=_rho, sigma=_sigma, parameter_names=None):
    """Return a dict.
    ...
    if adaptive:
        raise NotImplementedError('Adaptive parameters not implemented. ' + 'Set `adaptive` to False.')
    if any(x0_uncert == 0):
        raise ValueError('Uncertaities must not be 0.')
    options = {'Nparam': len(x0), 
       'maxiter': maxiter, 
       'maxfev': maxfev, 
    ...
    record = OptimizeResult({'time_beg': dt.datetime.now(), 
       'all_simplices': [], 'best_vertices': [], 'worst_vertices': [], 'all_fun_values': [], 'best_fun_values': [], 'worst_fun_values': [], 'lookup_table': table, 
       'action': []})
    ...
    while True:
        ...
        x_reflected = get_reflected_vertex(x_worst, x_centroid, options['alpha'])
        f_reflected = f(x_reflected)
        if f_reflected < f_best:
            x_expanded = get_expanded_vertex(x_reflected, x_centroid, options['gamma'])
            ...
            if f_reflected > f_worst:
                x_contracted = get_contracted_vertex(x_worst, x_centroid, options['rho'])
                f_contracted = f(x_contracted)
                if f_contracted < f_worst:
                    simplex[m_worst] = x_contracted
                    record['action'].append(_CONTRACT)
                else:
                    simplex = get_shrunk_simplex(simplex, x_centroid, m_best, options['sigma'])
...
if __name__ == '__main__':
    ...
    N = 9
    x0 = -np.array([2.2] * N)
    x0_uncert = np.array([5.0] * N)
    f = styblinski
    record = minimize_nelder_mead(f, x0, x0_uncert=x0_uncert, disp=True, maxfev=np.inf, maxiter=np.inf, maxtime_sec=maxtime_sec, fatol=1e-09)
