...
"""Correct target coordinates of airborne remote sensing data.
    ...
    them to the output file. The output is still on a (time, range) grid with
    ...
    Notes
    -----
    * Xt is aligned with the PROJECTION of the trajectory to a horizontal surface!
    * Xc is named X in [1]
...
def main(setup_file=_setup_file, overwrite=True):
    ...
    for fni in fnis:
        ...
        ###################################################
        # check whether input file can be skipped
        skip = True

        # only skip if output file already exists
        fno = io.paths.get_output_filename(fni, setup=setup)
        if not os.path.isfile(fno):
            skip = False

        # do not skip if overwriting is desired
        if overwrite:
            skip = False
        ...
        compute_sensor_coordinates(data, transforms)
        compute_sensor_attitude(data, transforms)
        compute_sensor_velocity(data, transforms)
        compute_distance_covered_by_sensor(data, transforms)
        compute_vertical_resolution(data, transforms)
        compute_target_coordinates(data, transforms)
...
def get_ins_data(date, setup):
    ...
    #
    # `data_ins` is a dict that itself contains dicts. E. g.
    ...
    # to INS data from their native time grid onto another one (to be specified
...
def create_coordinate_transforms(data, setup):
    """Return transforms as two dicts.
    ...
    # Xc -> Xg  (local Earth-relative -> geographic)
